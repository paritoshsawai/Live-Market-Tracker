...
import { PANEL_CATEGORY_MAP, ALL_PANELS, getEffectivePanelConfig, getInitialDefaultPanelKeys, isPortfolioVisiblePanel } from '@/config/panels';
...
export class UnifiedSettings {
  ...
  constructor(config: UnifiedSettingsConfig) {
    ...
    this.overlay.addEventListener('click', (e) => {
      ...
      if (target === this.overlay || target.closest('.unified-settings-close')) {
        this.close();
        return;
      }
  ...
  private getVisiblePanelEntries(): Array<[string, PanelConfig]> {
    let entries = Object.entries(this.draftPanelSettings)
      ...
      .filter(([key]) => isPortfolioVisiblePanel(key, SITE_VARIANT));

    if (this.activePanelCategory !== 'all') {
      const catDef = PANEL_CATEGORY_MAP[this.activePanelCategory];
      if (!catDef) return [];
      if (!this.categoryMatchesVariant(catDef)) return [];
  ...
  private clonePanelSettings(source: Record<string, PanelConfig> = this.config.getPanelSettings()): Record<string, PanelConfig> {
    ...
    for (const key of Object.keys(ALL_PANELS)) {
      if (!isPortfolioVisiblePanel(key, SITE_VARIANT)) continue;
      if (!(key in cloned)) {
        cloned[key] = { ...getEffectivePanelConfig(key, SITE_VARIANT), enabled: initialDefaults.has(key) };
      }
    }
    return cloned;
  }
  ...
  private getAvailableRegions(): Array<{ key: string; label: string }> {
    ...
    for (const [regionKey, regionDef] of Object.entries(SOURCE_REGION_MAP)) {
      ...
      if (regionDef.feedKeys.some((fk) => feedKeys.has(fk))) {
        regions.push({ key: regionKey, label: t(regionDef.labelKey) });
      }
    }
  ...
  private getVisibleSourceNames(): string[] {
    ...
    if (this.activeSourceRegion === 'all') {
      ...
      sources = this.getSourcesByRegion().get(this.activeSourceRegion) || [];
    }
